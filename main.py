...
def createEmoticon(update, context):
    ...
    driver = webdriver.Chrome(executable_path="/app/KakaoEmoticon2TelegramSticker/chromedriver", options=options)
    ...
    for srcEmoticon in itemEmoticons:
        ...
        urllib.request.urlretrieve(urlEmoticon, "/app/KakaoEmoticon2TelegramSticker/emoticonTemp/%d.png"%(count))

        img = Image.open("/app/KakaoEmoticon2TelegramSticker/emoticonTemp/%d.png"%(count))
        imgResize = img.resize((512, 512))
        imgResize.save("/app/KakaoEmoticon2TelegramSticker/emoticonTemp/%d.png"%(count))
        ...
        if count == 0:
            ...
            context.bot.create_new_sticker_set(user_id=318996831, 
                                                ...
                                                png_sticker=open("/app/KakaoEmoticon2TelegramSticker/emoticonTemp/0.png", "rb"))
        else:
            context.bot.add_sticker_to_set(user_id=318996831,
                                            name=stickerName,
                                            emojis="😀",
                                            png_sticker=open("/app/KakaoEmoticon2TelegramSticker/emoticonTemp/%d.png"%(count), "rb"))

        os.remove("/app/KakaoEmoticon2TelegramSticker/emoticonTemp/%d.png"%(count))
